...
class adminController extends Controller
{
    ...
    public function updateUserProfit() 
    {
        ...
        foreach ($funds as $fund) {
            if ($fund->balance >= 1000 && $fund->balance <= 999999) {
                // Find the appropriate investment plan for the user's balance
                foreach ($allplan as $plan) {
                    if ($plan->minimum <= $fund->balance && $fund->balance <= $plan->maximum) {
                        $roiPercentage = $plan->percentage;
                        $profit = ($fund->balance * $roiPercentage) / 100;
                        $fund->currentprofit += $profit;
                        $fund->save();
                    }
                }
            }
        ...
        return redirect()->route('adminindex')->with('success', 'Operation successfull');
    ...
    function account_view_user(Request $req)
    {
        ...
        $userdocs = KycFile::where('user_id', $req->id)->get();
...
// dd($userdocs);
    ...
    public function depositupdate(Request $request)
    {
        ...
        if ($result) {
            # code...
            return back()->with("success", "deposit updated was succesfuly");
        } else {
            # code...
    ...
    public function markwithdrawalpaid(Request $request)
    {
        ...
        if ($result) {
            ...
            if ($userwithdrawal->from == "dividend") {
                # code...
                $userfund->currentprofit = $userfund->currentprofit-$userwithdrawal->amount;
                $userfund->save();
            } else {
                # code...
                $userfund->bonus = $userfund->bonus-$userwithdrawal->amount;
                $userfund->save();
            }
